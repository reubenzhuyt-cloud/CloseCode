---
title: CLI
description: CloseCode CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
closecode
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、CloseCode をプログラム的に操作できるようになります。
...
CloseCode ターミナルユーザーインターフェイスを開始します。
...
CloseCode CLI には次のコマンドもあります。

---

### agent

CloseCode のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の CloseCode バックエンドサーバーにターミナルを接続します。

```bash
opencode attach [url]
```

これにより、リモート CloseCode バックエンドで TUI を使用できるようになります。例えば：
...
| <nobr><code>{"--username"}</code></nobr> | `-u`     | Basic 認証ユーザー名（デフォルトは `OPENCODE_SERVER_USERNAME` または `closecode`） |
...
CloseCode は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`closecode auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/opencode/auth.json` に保存されます。

```bash
closecode auth login
```

CloseCode が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを CloseCode で確認したい場合に便利です。
...
実行中の `closecode serve` インスタンスにアタッチして、実行ごとの MCP サーバーのコールドブート時間を回避することもできます。
...
API アクセスのためにヘッドレス CloseCode サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。

```bash
closecode serve
```

これにより、TUI インターフェイスを使用せずに opencode 機能への API アクセスを提供する HTTP サーバーが起動します。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `closecode`)。
...
CloseCode セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは CloseCode 共有 URL からセッションデータをインポートします。

```bash
closecode import <file>
```

ローカルファイルまたは CloseCode 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス CloseCode サーバーを起動します。

```bash
closecode web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて CloseCode にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `closecode`)。
...
CloseCode をアンインストールし、関連ファイルをすべて削除します。
...
| `OPENCODE_SERVER_USERNAME`            | 文字列   | 基本認証ユーザー名 (デフォルト `closecode`) をオーバーライドします。 |
