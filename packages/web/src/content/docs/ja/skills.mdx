...
エージェントスキルにより、CloseCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカルパスの場合、CloseCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
CloseCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
