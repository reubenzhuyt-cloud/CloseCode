---
title: CloseCode Zen
description: CloseCode によって提供されるモデルの厳選されたリスト。
---
...
CloseCode Zen は、CloseCode チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は CloseCode のほかのプロバイダーと同じように動作します。CloseCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、CloseCode を使うために必須ではありません。
...
:::tip
CloseCode でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
CloseCode Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

CloseCode Zen は、CloseCode のほかのプロバイダーと同じように動作します。

1. **<a href={console}>CloseCode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、CloseCode Zen を選んで API キーを貼り付けます。
...
CloseCode 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。CloseCode はこれらのモデルをセッションタイトルの生成に使用します。
...
- DeepSeek V4 Flash Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Laguna S 2.1 Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling-3.0-tiny Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- LongCat-2.0 Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で CloseCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で CloseCode で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが CloseCode Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに CloseCode では常にほかのプロバイダーも使えるようにすること。
