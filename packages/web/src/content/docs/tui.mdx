---
title: TUI
description: Using the CloseCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CloseCode starts the TUI for the current directory.
...
When using the CloseCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CloseCode. Allows you to select from available providers and add their API keys.
...
Exit CloseCode. _Aliases_: `/quit`, `/q`
...
- `leader_timeout` - Controls how long CloseCode waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `closecode.default`.
