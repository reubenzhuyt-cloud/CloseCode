...
Вы можете настроить параметры сервера для команд `closecode serve` и `closecode web` с помощью опции `server`.
...
Этот параметр применяется ко всем интерфейсам: TUI, CLI (`closecode run`), настольному приложению и действию GitHub.
