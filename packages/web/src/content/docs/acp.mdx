---
title: ACP Support
description: Use CloseCode in any ACP-compatible editor.
---

CloseCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use CloseCode via ACP, configure your editor to run the `closecode acp` command.

The command starts CloseCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'CloseCode' agent in the AI Chat agent selector.
...
To use CloseCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use CloseCode as the ACP agent for chat.
...
CloseCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your CloseCode config
