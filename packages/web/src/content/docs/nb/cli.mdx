---
title: CLI
description: CloseCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
closecode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med CloseCode programmatisk.
...
Start CloseCode-terminalbrukergrensesnittet.
...
CloseCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter for CloseCode.
...
Koble en terminal til en allerede kjørende CloseCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern CloseCode-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `closecode`) |
...
CloseCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `closecode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Når CloseCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i CloseCode.
...
Kjør CloseCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `closecode serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende CloseCode-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `closecode`) |
...
Start en headless CloseCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
closecode serve
```

Dette starter en HTTP-server som gir API tilgang til CloseCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `closecode`).
...
Vis tokenbruk og kostnadsstatistikk for CloseCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en CloseCode share-URL.

```bash
closecode import <file>
```

Du kan importere fra en lokal fil eller en CloseCode share-URL.
...
Start en headless CloseCode-server med et webgrensesnitt.

```bash
closecode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til CloseCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `closecode`).
...
Avinstaller CloseCode og fjern alle relaterte filer.
...
Oppdaterer CloseCode til siste versjon eller en spesifikk versjon.
...
CloseCode CLI bruker følgende globale flagg.
...
CloseCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `closecode`) |
