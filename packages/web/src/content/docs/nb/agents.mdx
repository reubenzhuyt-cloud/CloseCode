...
Det er to typer agenter i CloseCode; primære agenter og underagenter.
...
CloseCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
CloseCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

CloseCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker CloseCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale CloseCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i CloseCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [CloseCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `closecode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/reubenzhuyt-cloud/CloseCode).
:::
