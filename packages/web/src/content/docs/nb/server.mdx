---
title: Tjener
description: Samhandle med CloseCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `closecode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en CloseCode-klient kan bruke.
...
```bash
closecode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
closecode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `closecode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `closecode serve` og `closecode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password closecode serve
```
...
Når du kjører `closecode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk CloseCode-serveren til å samhandle med CloseCode programmatisk.
:::

Denne arkitekturen lar CloseCode støtte flere klienter og lar deg samhandle med CloseCode programmatisk.

Du kan kjøre `closecode serve` for å starte en frittstående server. Hvis du har
CloseCode TUI kjører, vil `closecode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av CloseCode [IDE](/docs/ide) plugins.
