...
CloseCode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre CloseCode starter TUI for gjeldende katalog.
...
Når du bruker CloseCode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til CloseCode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt CloseCode. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge CloseCode venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `closecode.default`.
