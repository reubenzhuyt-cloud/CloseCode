...
Du kan legge til eksterne verktøy i CloseCode ved å bruke _Model Context Protocol_ eller MCP. CloseCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [CloseCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
CloseCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil CloseCode:
...
Hvis serveren krever autentisering, vil CloseCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `closecode mcp auth <server-name>`.
...
```bash
closecode mcp auth my-oauth-server
```
...
```bash
closecode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil CloseCode lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
closecode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i CloseCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom CloseCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble CloseCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver CloseCode-samtale oppført som en separat økt.
