...
CloseCode støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke CloseCode via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `closecode acp`.

Kommandoen starter CloseCode som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'CloseCode'-agenten i AI Chat-agentvelgeren.
...
For å bruke CloseCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke CloseCode som ACP-agent for chat.
...
CloseCode fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i CloseCode-konfigurasjonen
