---
title: CLI
description: Opciones y comandos de la CLI de CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de CloseCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
closecode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con CloseCode mediante programación.
...
Inicie la interfaz de usuario del terminal CloseCode.
...
La CLI de CloseCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para CloseCode.
...
Conecte una terminal a un servidor backend CloseCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto CloseCode. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `closecode`) |
...
CloseCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `closecode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Cuando se inicia CloseCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en CloseCode.
...
También puede conectarse a una instancia `closecode serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
Inicie un servidor CloseCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
closecode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad opencode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `closecode`).
...
Administrar CloseCode sesiones.
...
Enumere todas las sesiones CloseCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones CloseCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida CloseCode.

```bash
closecode import <file>
```

Puede importar desde un archivo local o una URL compartida CloseCode.
...
Inicie un servidor CloseCode sin cabeza con una interfaz web.

```bash
closecode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a CloseCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `closecode`).
...
Desinstale CloseCode y elimine todos los archivos relacionados.
...
La CLI de CloseCode toma las siguientes banderas globales.
...
CloseCode se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `closecode`) |
