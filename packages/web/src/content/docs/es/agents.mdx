...
Hay dos tipos de agentes en CloseCode; agentes primarios y subagentes.
...
CloseCode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
CloseCode viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

CloseCode viene con dos agentes primarios integrados y tres subagentes integrados.
...
Si no se especifica ninguna temperatura, CloseCode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global CloseCode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración CloseCode usa el formato `provider/model-id`. Por ejemplo, si usa [CloseCode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `closecode models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/reubenzhuyt-cloud/CloseCode).
:::
