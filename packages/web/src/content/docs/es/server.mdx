...
El comando `closecode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente opencode puede usar.
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `closecode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `closecode serve` como a `closecode web`.
...
Cuando ejecuta `closecode`, inicia un TUI y un servidor. Donde el TUI es el
...
Puede ejecutar `closecode serve` para iniciar un servidor independiente. Si tienes el
opencode TUI ejecutándose, `closecode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos CloseCode [IDE](/docs/ide).
