---
title: Configuración
description: Usando la configuración CloseCode JSON.
---

Puede configurar CloseCode usando un archivo de configuración JSON.

---

## Formato

CloseCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Coloque su configuración global CloseCode en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia CloseCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Puede configurar los ajustes del servidor para los comandos `closecode serve` y `closecode web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor CloseCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración CloseCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, CloseCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración CloseCode a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, CloseCode recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`closecode run`), aplicación de escritorio y GitHub Acción.
...
CloseCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían CloseCode con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir CloseCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
