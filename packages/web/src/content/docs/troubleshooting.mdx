...
To debug issues with CloseCode, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `closecode --log-level DEBUG`.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\closecode`
...
To uninstall the CloseCode CLI and remove its related files, run:
...
To remove CloseCode Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

CloseCode Desktop runs a local CloseCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `CloseCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
CloseCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so CloseCode can rebuild it.

1. Quit CloseCode Desktop completely.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\closecode`

3. Restart CloseCode Desktop.

---

### Fix server connection issues

CloseCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, CloseCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with CloseCode's features.

---

### Notifications not showing

CloseCode Desktop only shows system notifications when:

- notifications are enabled for CloseCode in your OS settings, and
...
1. Quit CloseCode Desktop.
2. Find and delete these files (they live in the CloseCode Desktop app data directory):
...
If you're experiencing issues with CloseCode:
...
### CloseCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `closecode upgrade`
...
To figure out what models you have access to, run `closecode models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\closecode`
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\closecode`
