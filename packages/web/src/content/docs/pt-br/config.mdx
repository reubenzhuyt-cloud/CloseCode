...
Você pode configurar as configurações do servidor para os comandos `closecode serve` e `closecode web` através da opção `server`.
...
Essa configuração se aplica a todas as interfaces: TUI, CLI (`closecode run`), aplicativo desktop e GitHub Action.
