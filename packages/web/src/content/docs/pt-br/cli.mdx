...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `closecode`) |
...
O opencode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `closecode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/opencode/auth.json`.
...
```bash "closecode run"
closecode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `closecode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
closecode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
Isso inicia um servidor HTTP que fornece acesso à funcionalidade do opencode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `closecode`).
...
Isso inicia um servidor HTTP e abre um navegador para acessar o opencode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `closecode`).
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `closecode`) |
