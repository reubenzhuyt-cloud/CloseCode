...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `closecode --log-level DEBUG`.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\closecode`
...
- Apenas macOS: menu `CloseCode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\closecode`
...
3. Certifique-se de que você tem a versão mais recente com `closecode upgrade`
...
Para descobrir quais modelos você tem acesso, execute `closecode models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\closecode`
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\closecode`
