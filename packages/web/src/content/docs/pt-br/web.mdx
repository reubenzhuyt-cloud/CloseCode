...
Para a melhor experiência, execute `closecode web` a partir do [WSL](/docs/windows-wsl) em vez do PowerShell. Isso garante acesso adequado ao sistema de arquivos e integração com o terminal.
:::
...
O nome de usuário padrão é `closecode`, mas pode ser alterado com `OPENCODE_SERVER_USERNAME`.
