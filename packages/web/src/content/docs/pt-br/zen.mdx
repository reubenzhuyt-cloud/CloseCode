---
title: Zen
description: Lista selecionada de modelos fornecidos pelo CloseCode.
---
...
O CloseCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do CloseCode.

O Zen funciona como qualquer outro provedor no CloseCode. Você faz login no CloseCode Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o CloseCode.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o CloseCode.
:::
...
O CloseCode Zen é um gateway de AI que dá a você acesso a esses modelos.

---

## Como funciona

O CloseCode Zen funciona como qualquer outro provedor no CloseCode.

1. Você faz login no **<a href={console}>CloseCode Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona CloseCode Zen e cola sua chave de API.
...
O [model id](/docs/config/#models) na sua configuração do CloseCode usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O CloseCode usa esses modelos para gerar títulos de sessões.
...
- DeepSeek V4 Flash Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- MiMo-V2.5 Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Laguna S 2.1 Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Ling-3.0-tiny Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- LongCat-2.0 Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- North Mini Code Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Ultra Free está disponível no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no CloseCode por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o CloseCode também.
