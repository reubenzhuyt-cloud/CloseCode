...
O opencode integra-se com VS Code, Cursor ou qualquer IDE que suporte um terminal. Basta executar `closecode` no terminal para começar.
...
3. Execute `closecode` - a extensão será instalada automaticamente
...
- Certifique-se de que você está executando `closecode` no terminal integrado.
