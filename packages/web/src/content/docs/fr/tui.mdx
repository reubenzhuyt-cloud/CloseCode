---
title: TUI
description: Utilisation de l’interface utilisateur du terminal CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de CloseCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le CloseCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à CloseCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez CloseCode. *Alias* : `/quit`, `/q`
...
- `leader_timeout` - Contrôle la durée pendant laquelle CloseCode attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `closecode.default`.
