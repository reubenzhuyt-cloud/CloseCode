...
Pour déboguer les problèmes avec CloseCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `closecode --log-level DEBUG`.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\closecode`
...
CloseCode Desktop exécute un serveur CloseCode local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `CloseCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
CloseCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que CloseCode puisse le reconstruire.

1. Quittez complètement CloseCode Desktop.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\closecode`

3. Redémarrez le bureau CloseCode.

---

### Résoudre les problèmes de connexion au serveur

CloseCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Sur Windows, CloseCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de CloseCode.

---

### Les notifications ne s'affichent pas

CloseCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour CloseCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau CloseCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application CloseCode Desktop) :
...
Si vous rencontrez des problèmes avec CloseCode :
...
3. Assurez-vous d'avoir la dernière version avec `closecode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `closecode models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\closecode`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\closecode`
