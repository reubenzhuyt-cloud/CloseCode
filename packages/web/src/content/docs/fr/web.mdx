---
title: Web
description: En utilisant CloseCode dans votre navigateur.
---

CloseCode peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![CloseCode Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement CloseCode dans votre navigateur par défaut.
...
Pour une expérience optimale, exécutez `closecode web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Par défaut, CloseCode sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre CloseCode accessible sur votre réseau :

```bash
closecode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, CloseCode affichera les adresses locales et réseau :
...
Le nom d'utilisateur par défaut est `closecode` mais peut être modifié avec `OPENCODE_SERVER_USERNAME`.

---

## Utilisation de l'interface Web

Une fois démarrée, l'interface web permet d'accéder à vos sessions CloseCode.
...
![CloseCode Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![CloseCode Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)
