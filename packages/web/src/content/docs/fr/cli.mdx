---
title: CLI
description: Options et commandes de la CLI CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI CloseCode démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
closecode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec CloseCode par programme.
...
Démarrez l’interface utilisateur du terminal CloseCode.
...
La CLI CloseCode dispose également des commandes suivantes.

---

### agent

Gérer les agents pour CloseCode.
...
Attachez un terminal à un serveur backend CloseCode déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opencode attach [url]
```

Cela permet d'utiliser le TUI avec un backend CloseCode distant. Par exemple :
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nom d'utilisateur d'authentification de base (par défaut `OPENCODE_SERVER_USERNAME` ou `closecode`) |
...
CloseCode est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `closecode auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Lorsque CloseCode démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans CloseCode.
...
Vous pouvez également vous connecter à une instance `closecode serve` en cours d'exécution pour éviter les temps de démarrage à froid du serveur MCP à chaque exécution :
...
Démarrez un serveur CloseCode sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
closecode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'CloseCode sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `closecode`).
...
Liste toutes les sessions CloseCode.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions CloseCode.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage CloseCode.

```bash
closecode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage CloseCode.
...
Démarrez un serveur CloseCode sans interface graphique avec une interface Web.

```bash
closecode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à CloseCode via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `closecode`).
...
Désinstallez CloseCode et supprimez tous les fichiers associés.
...
CloseCode peut être configuré à l'aide de variables d'environnement.
...
| `OPENCODE_SERVER_USERNAME`            | chaîne  | Remplacer le nom d'utilisateur d'authentification de base (par défaut `closecode`) |
