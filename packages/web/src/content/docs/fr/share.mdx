---
title: Partager
description: Partagez vos conversations CloseCode.
---

La fonctionnalité de partage de CloseCode vous permet de créer des liens publics vers vos conversations CloseCode, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, CloseCode :
...
CloseCode prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, CloseCode utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
