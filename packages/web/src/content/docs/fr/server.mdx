...
La commande `closecode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opencode peut utiliser.
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `closecode`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `closecode serve` et à `closecode web`.
...
Lorsque vous exécutez `closecode`, il démarre un TUI et un serveur. Où le TUI est le
...
Vous pouvez exécuter `closecode serve` pour démarrer un serveur autonome. Si vous avez le
opencode TUI en cours d'exécution, `closecode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins CloseCode [IDE](/docs/ide).
