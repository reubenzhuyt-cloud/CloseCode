---
title: CLI
description: CloseCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
closecode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med CloseCode programmatisk.
...
Start CloseCode terminalbrugergrænsefladen.
...
Administrer agenter i CloseCode.
...
Tilslut en terminal til en allerede kørende CloseCode backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern CloseCode backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `closecode`) |
...
CloseCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `closecode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Når CloseCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i CloseCode.
...
Du kan også vedhæfte til en kørende `closecode serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
Start en hovedløs CloseCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
closecode serve
```

Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `closecode`).
...
Administrator CloseCode sessionsholder.
...
Liste over alle CloseCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine CloseCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller CloseCode del URL.
...
Start en hovedløs CloseCode-server med en webgrænseflade.

```bash
closecode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til CloseCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `closecode`).
...
Afinstaller CloseCode og fjern alle relaterede filer.
...
Opdaterer CloseCode til den nyeste version eller en specifik version.
...
CloseCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `closecode`) |
