...
Kommandoen `closecode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `closecode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `closecode serve` og `closecode web`.
...
Når du kører `closecode` starter den en TUI og en server. Hvor TUI er
...
Du kan køre `closecode serve` for at starte en selvstændig server. Hvis du har
opencode TUI kørende, vil `closecode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af CloseCode [IDE](/docs/ide) plugins.
...
CloseCode-serveren viser følgende API'er.
