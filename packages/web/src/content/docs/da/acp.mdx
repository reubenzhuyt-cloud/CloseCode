...
description: Brug CloseCode i enhver ACP-kompatibel editor.
---

CloseCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge CloseCode via ACP, konfigurer din editor til at køre kommandoen `closecode acp`.

Kommandoen starter CloseCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'CloseCode'-agent i AI Chat-agentvælgeren.
...
For at bruge CloseCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge CloseCode som ACP-agent til chat.
...
CloseCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din CloseCode-konfiguration
