...
For at fejlfinde problemer med CloseCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `closecode --log-level DEBUG`.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\closecode`
...
CloseCode Desktop kører en lokal CloseCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `CloseCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
CloseCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så CloseCode kan genopbygge den.

1. Afslut CloseCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/opencode`
- **Linux**: slet `~/.cache/opencode` (eller kør `rm -rf ~/.cache/opencode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\closecode`

3. Start CloseCode Desktop på ny.

---

### Løs problemer med serverforbindelse

CloseCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver CloseCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i CloseCode.

---

### Meddelelser vises ikke

CloseCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for CloseCode i dine OS-indstillinger, og
...
1. Afslut CloseCode Desktop.
2. Find og slet disse filer (de findes i CloseCode Desktop-appens datamappe):
...
Hvis du oplever problemer med CloseCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `closecode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\closecode`
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\closecode`
