---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i CloseCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CloseCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i CloseCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i CloseCode-konfigurationen.
...
CloseCode Zen er en liste over modeller leveret af CloseCode-teamet, som er blevet testet og verificeret til at fungere godt med CloseCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med CloseCode Zen.
:::
...
Det fungerer som alle andre udbydere i CloseCode og er helt valgfrit at bruge.

---

## CloseCode Go

CloseCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af CloseCode-teamet, som er testet og verificeret til at fungere godt med CloseCode.

1. Kør kommandoen `/connect` i TUI, vælg `CloseCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med CloseCode:
...
:::info
At bruge dit Claude Pro/Max-abonnement i CloseCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
gemme tokenet i closecode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
CloseCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse CloseCode
...
Sådan bruger du GitHub Copilot-abonnementet med CloseCode:
...
Sådan bruger du Google Vertex AI med CloseCode:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver CloseCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver CloseCode-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for CloseCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med CloseCode:
...
3. Kopier API-nøglen til brug i CloseCode.
...
6. **Vigtigt**: Før du bruger skymodeller i CloseCode, skal du hente modelinformationen lokalt:
...
CloseCode Zen er en liste over testede og verificerede modeller leveret af CloseCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>CloseCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **CloseCode Zen**.

   ```txt
   /connect
   ```

3. Indtast CloseCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med CloseCode:
...
`limit`-felterne lader CloseCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `closecode auth list` for at se om legitimationsoplysningerne
