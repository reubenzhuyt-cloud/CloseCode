---
title: เซน
description: รายการโมเดลที่คัดสรรโดย CloseCode
---
...
CloseCode Zen คือรายการโมเดลที่ผ่านการทดสอบและยืนยันแล้วโดยทีม CloseCode

Zen ทำงานเหมือน provider อื่น ๆ ใน CloseCode คุณล็อกอินเข้า CloseCode Zen แล้วรับ
...
:::tip
เราได้ทดสอบโมเดลและ provider ที่คัดมาแล้วกลุ่มหนึ่งซึ่งทำงานได้ดีกับ CloseCode
:::
...
CloseCode Zen คือ AI gateway ที่ให้คุณเข้าถึงโมเดลเหล่านี้

---

## วิธีการทำงาน

CloseCode Zen ทำงานเหมือน provider อื่น ๆ ใน CloseCode

1. คุณลงชื่อเข้าใช้ **<a href={console}>CloseCode Zen</a>** เพิ่มรายละเอียดการเรียกเก็บเงิน แล้วคัดลอก API key ของคุณ
2. รันคำสั่ง `/connect` ใน TUI เลือก CloseCode Zen แล้ววาง API key ของคุณ
...
[model id](/docs/config/#models) ใน CloseCode config ของคุณใช้รูปแบบ `opencode/<model-id>` ตัวอย่างเช่น สำหรับ GPT 5.5 คุณจะใช้ `opencode/gpt-5.5` ใน config ของคุณ
...
คุณอาจสังเกตเห็น[โมเดลต้นทุนต่ำ](/docs/config/#models) เช่น Haiku, Nano หรือ Flash ในประวัติการใช้งานของคุณ CloseCode ใช้โมเดลเหล่านี้เพื่อสร้างชื่อเซสชัน
...
- DeepSeek V4 Flash Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- MiMo-V2.5 Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- Laguna S 2.1 Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- Ling-3.0-tiny Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- LongCat-2.0 Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- North Mini Code Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- Nemotron 3 Ultra Free เปิดให้ใช้บน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
- Big Pickle เป็น stealth model ที่ใช้งานฟรีบน CloseCode ในช่วงเวลาจำกัด ทีมกำลังใช้ช่วงเวลานี้เพื่อเก็บ feedback และปรับปรุงโมเดล
...
เราสร้าง CloseCode Zen ขึ้นมาเพื่อ:
...
4. ทำให้เกิด **no lock-in** โดยให้คุณใช้มันร่วมกับ coding agent อื่น ๆ ได้ และยังเปิดให้คุณใช้ provider อื่น ๆ กับ CloseCode ได้เสมอ
