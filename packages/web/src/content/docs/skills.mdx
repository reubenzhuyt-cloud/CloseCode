...
Agent skills let CloseCode discover reusable instructions from your repo or home directory.
...
CloseCode searches these locations:
...
For project-local paths, CloseCode walks up from your current working directory until it reaches the git worktree.
...
Put CloseCode-specific behavior in `metadata` instead of adding custom top-level fields.

CloseCode recognizes these optional `metadata` keys:
...
CloseCode also accepts YAML booleans like `opencode/slash: true` and `opencode/autoinvoke: false`.
...
CloseCode lists available skills in the `skill` tool description.
