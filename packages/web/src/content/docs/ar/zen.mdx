---
title: Zen
description: قائمة منتقاة من النماذج يوفّرها CloseCode.
---
...
CloseCode Zen هي قائمة بالنماذج التي اختبرها فريق CloseCode وتحقّق منها.

يعمل Zen مثل أي مزوّد آخر في CloseCode. تسجّل الدخول إلى CloseCode Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام CloseCode.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع CloseCode.
:::
...
CloseCode Zen هي بوابة AI تتيح لك الوصول إلى هذه النماذج.

---

## كيف يعمل

يعمل CloseCode Zen مثل أي مزوّد آخر في CloseCode.

1. تسجّل الدخول إلى **<a href={console}>CloseCode Zen</a>**، وتضيف تفاصيل الفوترة، ثم تنسخ مفتاح API.
2. تشغّل الأمر `/connect` في واجهة TUI، وتختار CloseCode Zen، ثم تلصق مفتاح API.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات CloseCode الصيغة `opencode/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `opencode/gpt-5.5` في إعداداتك.
...
قد تلاحظ [نماذج منخفضة التكلفة](/docs/config/#models)، مثل Haiku أو Nano أو Flash، في سجل الاستخدام. يستخدم CloseCode هذه النماذج لإنشاء عناوين الجلسات.
...
- DeepSeek V4 Flash Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- MiMo-V2.5 Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Laguna S 2.1 Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Ling-3.0-tiny Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- LongCat-2.0 Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- North Mini Code Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3 Ultra Free متاح على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Big Pickle نموذج خفي ومتاح مجانا على CloseCode لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع CloseCode أيضا دائما.
