---
title: CLI
description: أوامر وخيارات CLI في CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ CloseCode افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
closecode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع CloseCode برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ CloseCode.
...
يتضمن CLI الخاص بـ CloseCode أيضا الأوامر التالية.

---

### agent

إدارة وكلاء CloseCode.
...
إرفاق terminal بخادم الواجهة الخلفية لـ CloseCode قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
opencode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ CloseCode تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `closecode`) |
...
يعتمد CloseCode على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `closecode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

عند تشغيل CloseCode يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في CloseCode.
...
يمكنك أيضا الإرفاق بمثيل `closecode serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
بدء خادم CloseCode بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
closecode serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف opencode دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `closecode`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات CloseCode لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة CloseCode.

```bash
closecode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة CloseCode.
...
بدء خادم CloseCode بدون واجهة مع واجهة ويب.

```bash
closecode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى CloseCode عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `closecode`).
...
إلغاء تثبيت CloseCode وإزالة جميع الملفات ذات الصلة.
...
يمكن تهيئة CloseCode باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `closecode`) |
