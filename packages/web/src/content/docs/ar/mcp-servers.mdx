...
يمكنك إضافة أدوات خارجية إلى CloseCode باستخدام _Model Context Protocol_، أو MCP. يدعم CloseCode خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات CloseCode](https://opencode.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى CloseCode تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم CloseCode بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك CloseCode المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `closecode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن CloseCode الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل CloseCode إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات CloseCode مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط CloseCode بحساب Sentry الخاص بك.
