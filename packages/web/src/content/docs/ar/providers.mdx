---
title: المزوّدون
description: استخدام أي موفّر LLM في CloseCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم CloseCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات CloseCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات CloseCode.
...
CloseCode Zen هي قائمة نماذج يوفّرها فريق CloseCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع CloseCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع CloseCode Zen.
:::
...
يعمل مثل أي مزوّد آخر في CloseCode واستخدامه اختياري بالكامل.

---

## CloseCode Go

CloseCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق CloseCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع CloseCode.

1. شغّل الأمر `/connect` في TUI، واختر `CloseCode Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع CloseCode:
...
:::info
استخدام اشتراك Claude Pro/Max داخل CloseCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم CloseCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر CloseCode
...
لاستخدام Google Vertex AI مع CloseCode:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة CloseCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة CloseCode مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع CloseCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع CloseCode:
...
3. انسخ مفتاح API لاستخدامه في CloseCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في CloseCode، يجب سحب معلومات النموذج محليا:
...
CloseCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق CloseCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>CloseCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **CloseCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ CloseCode.
...
تسمح حقول `limit` لـ CloseCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `closecode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
