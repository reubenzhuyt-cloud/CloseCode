...
You can add external tools to CloseCode using the _Model Context Protocol_, or MCP. CloseCode supports both local and remote servers.
...
You can define MCP servers in your [CloseCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
CloseCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CloseCode will:
...
If the server requires authentication, CloseCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `closecode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, CloseCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Tools from your MCP servers are available in CloseCode alongside built-in tools. You can manage them through the CloseCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect CloseCode to your Sentry account.
