...
opencode, VS Code, Cursor veya bir terminali destekleyen herhangi bir IDE ile entegre olur. Başlamak için terminalde `closecode` komutunu çalıştırmanız yeterli.
...
3. `closecode` komutunu çalıştırın - uzantı otomatik olarak yüklenir
...
- Entegre terminalde `closecode` çalıştırdığınızdan emin olun.
