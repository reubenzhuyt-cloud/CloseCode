...
CloseCode Zen, opencode ekibi tarafından sağlanan modellerin bir listesidir.
opencode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz CloseCode Zen ile başlamanızı öneririz.
:::
...
CloseCode Go, opencode ile iyi çalıştığı test edilmiş ve doğrulanmış, opencode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `CloseCode Go`'yu seçin ve [opencode.ai/auth](https://opencode.ai/zen) adresine gidin.
...
CloseCode Zen, opencode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>CloseCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **CloseCode Zen**'i arayın.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `closecode auth list` komutunu çalıştırın.
