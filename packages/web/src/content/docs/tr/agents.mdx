...
CloseCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

CloseCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
opencode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [CloseCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `closecode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/reubenzhuyt-cloud/CloseCode).
:::
