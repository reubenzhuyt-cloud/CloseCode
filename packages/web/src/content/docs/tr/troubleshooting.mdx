...
Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `closecode --log-level DEBUG`.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\closecode` yapistirin
...
- Yalnizca macOS: `closecode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\closecode` yapistirin
...
3. `closecode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `closecode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\closecode`
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\closecode`
