...
En iyi deneyim için PowerShell yerine `closecode web`'yi [WSL](/docs/windows-wsl)'den çalıştırın. Bu, uygun dosya sistemi erişimini ve terminal entegrasyonunu sağlar.
:::
...
Kullanıcı adı varsayılan olarak `closecode` şeklindedir ancak `OPENCODE_SERVER_USERNAME` ile değiştirilebilir.
