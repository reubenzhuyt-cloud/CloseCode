...
`closecode serve` ve `closecode web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`closecode run`), masaüstü uygulaması ve GitHub Action.
