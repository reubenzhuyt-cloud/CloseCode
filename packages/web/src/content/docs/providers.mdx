---
title: Providers
description: Using any LLM provider in CloseCode.
---

import config from "../../../config.mjs"
export const console = config.console

CloseCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CloseCode config.
...
You can customize the providers through the `provider` section in your CloseCode
config.
...
CloseCode Zen is a list of models provided by the CloseCode team that have been
tested and verified to work well with CloseCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CloseCode Zen.
:::

1. Run the `/connect` command in the TUI, select `CloseCode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in CloseCode and is completely optional to use.

---

## CloseCode Go

CloseCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the CloseCode team that have been
tested and verified to work well with CloseCode.

1. Run the `/connect` command in the TUI, select `CloseCode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with CloseCode:
...
CloseCode. Anthropic explicitly prohibits this.

Previous versions of CloseCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in CloseCode with zero setup:
...
CloseCode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; CloseCode uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize CloseCode. Sign in and approve.

   :::note
   CloseCode requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, CloseCode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — CloseCode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in CloseCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

CloseCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in closecode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
CloseCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock CloseCode
...
When a `duo-workflow-*` model is selected, CloseCode will:
...
4. Route tool execution requests through CloseCode's permission-gated tool system
...
To use GMI Cloud with CloseCode:
...
To use Google Vertex AI with CloseCode:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each CloseCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CloseCode conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for CloseCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with CloseCode:
...
3. Copy the API key for use in CloseCode.
...
6. **Important**: Before using cloud models in CloseCode, you must pull the model information locally:
...
CloseCode Zen is a list of tested and verified models provided by the CloseCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CloseCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CloseCode Zen**.

   ```txt
   /connect
   ```

3. Enter your CloseCode API key.
...
5. Complete sign-in in the browser that opens. CloseCode captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
2. Select **SuperGrok Subscription**. CloseCode opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by CloseCode. CloseCode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
CloseCode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
The `limit` fields allow CloseCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `closecode auth list` to see if the credentials
