...
Der Befehl `closecode serve` startet einen headless HTTP-Server.
...
Der Benutzername ist standardmaessig `closecode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `closecode serve` und `closecode web`.
...
Wenn du `closecode` startest, werden TUI und Server gestartet.
...
Mit `closecode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `closecode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die CloseCode-[IDE](/docs/ide)-Plugins.
