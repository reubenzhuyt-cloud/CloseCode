...
description: Verwenden Sie CloseCode in jedem ACP-kompatiblen Editor.
---

CloseCode unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um CloseCode über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `closecode acp`.

Der Befehl startet CloseCode als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „CloseCode“ in der AI Chat Agent Selector.
...
Um CloseCode als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass CloseCode als ACP-Agent für den Chat verwendet wird.
...
CloseCode funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer CloseCode-Konfiguration konfiguriert sind
