---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in CloseCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CloseCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer CloseCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem CloseCode anpassen
config.
...
CloseCode Zen ist eine Liste von Modellen, die vom CloseCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit CloseCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit CloseCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie CloseCode aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in CloseCode und ist völlig optional.

---

## CloseCode Go

CloseCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom CloseCode-Team getestet und verifiziert wurden, dass sie gut mit CloseCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `CloseCode Go` und gehen Sie zu [opencode.ai/auth](https://opencode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit CloseCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie CloseCode ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY closecode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile closecode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX closecode
   ```
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in CloseCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit CloseCode ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX closecode
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX closecode
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre CloseCode-Konfiguration hinzufügen.
...
um das Token im CloseCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
CloseCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von CloseCode
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit CloseCode:
...
So verwenden Sie Google Vertex AI mit CloseCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie CloseCode ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id closecode
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über CloseCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede CloseCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede CloseCode-Konversation als separate Sitzung aufgeführt.
...
Sie können CloseCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können CloseCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können CloseCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für CloseCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit CloseCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in CloseCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in CloseCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
CloseCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom CloseCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>CloseCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **CloseCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren CloseCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre CloseCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre CloseCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' closecode
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group closecode
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit CloseCode:
...
Sie können Modelle auch über Ihre CloseCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit CloseCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann CloseCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `closecode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die CloseCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer CloseCode-Konfiguration übereinstimmt.
