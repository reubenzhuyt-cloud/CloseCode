---
title: Web
description: Verwenden Sie CloseCode in Ihrem Browser.
---

CloseCode kann als Webanwendung in Ihrem Browser ausgeführt werden und bietet das gleiche leistungsstarke KI-Codierungserlebnis, ohne dass ein Terminal erforderlich ist.

![CloseCode Web – Neue Sitzung](../../../assets/web/web-homepage-new-session.png)
...
Dadurch wird ein lokaler Server auf `127.0.0.1` mit einem zufällig verfügbaren Port gestartet und CloseCode automatisch in Ihrem Standardbrowser geöffnet.
...
:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `closecode web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Standardmäßig wählt CloseCode einen verfügbaren Port aus. Sie können einen Port angeben:
...
Standardmäßig bindet der Server an `127.0.0.1` (nur localhost). So machen Sie CloseCode in Ihrem Netzwerk zugänglich:

```bash
closecode web --hostname 0.0.0.0
```

Bei Verwendung von `0.0.0.0` zeigt CloseCode sowohl lokale als auch Netzwerkadressen an:
...
Der Benutzername ist standardmäßig `closecode`, kann aber mit `OPENCODE_SERVER_USERNAME` geändert werden.

---

## Verwenden der Weboberfläche

Nach dem Start bietet die Weboberfläche Zugriff auf Ihre CloseCode-Sitzungen.

### Sitzungen

Zeigen Sie Ihre Sitzungen auf der Startseite an und verwalten Sie sie. Sie können aktive Sitzungen sehen und neue starten.

![CloseCode Web – Aktive Sitzung](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klicken Sie auf „Server anzeigen“, um verbundene Server und deren Status anzuzeigen.

![CloseCode Web – Siehe Server](../../../assets/web/web-homepage-see-servers.png)
