...
Wenn CloseCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `closecode --log-level DEBUG`.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\closecode`
...
CloseCode Desktop startet im Hintergrund einen lokalen CloseCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `CloseCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
CloseCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit CloseCode ihn neu aufbauen kann.

1. Beende CloseCode Desktop komplett.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\closecode`

3. Starte CloseCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

CloseCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt CloseCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit CloseCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

CloseCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer CloseCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende CloseCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von CloseCode Desktop):
...
Wenn du Probleme mit CloseCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `closecode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\closecode`
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\closecode`
