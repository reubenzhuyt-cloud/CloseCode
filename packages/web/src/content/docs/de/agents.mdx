...
Es gibt zwei Arten von Agenten in CloseCode; Primäragenten und Subagenten.
...
CloseCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Wenn keine Temperatur angegeben ist, verwendet CloseCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale CloseCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer CloseCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [CloseCode Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `closecode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/reubenzhuyt-cloud/CloseCode).
:::
