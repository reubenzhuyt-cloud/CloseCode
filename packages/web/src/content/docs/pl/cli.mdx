---
title: CLI
description: Opcje i polecenia wiersza poleceń CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI CloseCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
closecode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać CloseCode programowo.
...
Uruchamia interfejs terminalowy (TUI) CloseCode.
...
Interfejs CLI CloseCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami CloseCode.
...
Dołącz terminal do już działającego serwera backendu CloseCode uruchomionego za pomocą `serve` lub `web`.

```bash
opencode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem CloseCode. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `closecode`) |
...
CloseCode integruje się z wieloma dostawcami. Możesz użyć `closecode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Podczas uruchamiania CloseCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w CloseCode.
...
Uruchom CloseCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `closecode serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera CloseCode (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `closecode`) |
...
Uruchom serwer CloseCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
closecode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API CloseCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `closecode`).
...
Lista wszystkich sesji CloseCode.
...
Pokaż statystyki wykorzystania tokenów i sesji CloseCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania CloseCode.
...
Uruchom serwer CloseCode z interfejsem internetowym.

```bash
closecode web
```

Uruchamia to serwer HTTP i udostępnia CloseCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `closecode`).
...
Odinstaluj CloseCode i usuń wszystkie powiązane pliki.
...
Aktualizuj CloseCode do najnowszej lub określonej wersji.
...
Interfejs CLI CloseCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
CloseCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `closecode`)    |
