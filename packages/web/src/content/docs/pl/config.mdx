---
title: Konfiguracja
description: Ustawienia CloseCode w pliku JSON.
---

Możesz dostosować CloseCode za pomocą pliku konfiguracyjnego JSON.

---

## Format

CloseCode obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Umieść swoją globalną konfigurację CloseCode w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Kiedy CloseCode się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz skonfigurować serwer dla `closecode serve` i `closecode web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer CloseCode.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie CloseCode wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, CloseCode powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`closecode run`), aplikacji desktopowej i GitHub Actions.
...
CloseCode może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy CloseCode nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie CloseCode **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają CloseCode o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie CloseCode tylko do określonych dostawców.
