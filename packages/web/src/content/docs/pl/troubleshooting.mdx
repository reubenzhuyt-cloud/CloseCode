...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `closecode --log-level DEBUG`.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\closecode`
...
- Tylko macOS: menu `CloseCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\closecode`
...
3. Upewnij się, że masz najnowszą wersję z `closecode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `closecode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\closecode`
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\closecode`
