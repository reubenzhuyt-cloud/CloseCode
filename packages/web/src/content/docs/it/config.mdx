...
description: Usa la configurazione JSON di CloseCode.
---

Puoi configurare CloseCode usando un file di configurazione JSON.

---

## Formato

CloseCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Metti la configurazione globale di CloseCode in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando CloseCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Puoi configurare le impostazioni del server per i comandi `closecode serve` e `closecode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server CloseCode.
...
Puoi configurare provider e modelli da usare in CloseCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, CloseCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, CloseCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`closecode run`), app desktop e GitHub Action.
...
CloseCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono CloseCode con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare CloseCode a usare solo alcuni provider invece di disabilitarli uno per uno.
