...
Per diagnosticare problemi con CloseCode, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `closecode --log-level DEBUG`.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\closecode`
...
CloseCode Desktop esegue in background un server locale di CloseCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `CloseCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
CloseCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che CloseCode possa ricostruirla.

1. Chiudi completamente CloseCode Desktop.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\closecode`

3. Riavvia CloseCode Desktop.

---

### Risolvi problemi di connessione al server

CloseCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, CloseCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di CloseCode.

---

### Notifiche non visualizzate

CloseCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per CloseCode nelle impostazioni del sistema operativo, e
...
1. Chiudi CloseCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app CloseCode Desktop):
...
Se riscontri problemi con CloseCode:
...
Per capire a quali modelli hai accesso, esegui `closecode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\closecode`
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\closecode`
