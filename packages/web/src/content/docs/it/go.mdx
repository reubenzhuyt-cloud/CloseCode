...
CloseCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in CloseCode. Ti abboni a CloseCode Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare CloseCode.
...
CloseCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

CloseCode Go funziona come qualsiasi altro provider in CloseCode.

1. Accedi a **<a href={console}>CloseCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `CloseCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a CloseCode Go.
:::
...
CloseCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua CloseCode config
...
Abbiamo creato CloseCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con CloseCode.
