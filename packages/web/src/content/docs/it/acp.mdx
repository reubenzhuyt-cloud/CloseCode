---
title: Supporto ACP
description: Usa CloseCode in qualunque editor compatibile con ACP.
---

CloseCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare CloseCode via ACP, configura l'editor in modo che esegua il comando `closecode acp`.

Il comando avvia CloseCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'CloseCode' nel selettore agenti di AI Chat.
...
Per usare CloseCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare CloseCode come agente ACP per la chat.
...
CloseCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione CloseCode
