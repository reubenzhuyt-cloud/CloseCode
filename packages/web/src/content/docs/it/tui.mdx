---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire CloseCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di CloseCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a CloseCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da CloseCode. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo CloseCode attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `closecode.default`.
