---
title: CLI
description: Opzioni e comandi della CLI di CloseCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di CloseCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
closecode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con CloseCode in modo programmatico.
...
Avvia l'interfaccia testuale di CloseCode (interfaccia utente da terminale).
...
La CLI di CloseCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per CloseCode.
...
Collega un terminale a un backend server di CloseCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend CloseCode remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `closecode`) |
...
CloseCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `closecode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Quando CloseCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in CloseCode.
...
Puoi anche collegarti a una istanza `closecode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
Avvia un server CloseCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
closecode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `closecode`).
...
Gestisci le sessioni CloseCode.
...
Elenca tutte le sessioni CloseCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni CloseCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione CloseCode.

```bash
closecode import <file>
```

Puoi importare da un file locale o da un URL di condivisione CloseCode.
...
Avvia un server CloseCode headless con interfaccia web.

```bash
closecode web
```

Avvia un server HTTP e apre un browser per accedere a CloseCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `closecode`).
...
Disinstalla CloseCode e rimuove tutti i file correlati.
...
CloseCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `closecode`)     |
