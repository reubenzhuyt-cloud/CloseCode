...
Puoi aggiungere strumenti esterni a CloseCode usando il _Model Context Protocol_, o MCP. CloseCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [CloseCode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
CloseCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, CloseCode:
...
Se il server richiede autenticazione, CloseCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `closecode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, CloseCode memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in CloseCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione CloseCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare CloseCode al tuo account Sentry.
