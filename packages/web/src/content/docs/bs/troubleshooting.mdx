...
Da biste otklonili probleme s CloseCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `closecode --log-level DEBUG`.

---

## Pohrana

CloseCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\closecode`
...
CloseCode Desktop pokreće lokalni CloseCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `CloseCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
CloseCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga CloseCode mogao ponovo izgraditi.

1. Potpuno zatvorite CloseCode Desktop.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\closecode`

3. Ponovo pokrenite CloseCode Desktop.

---

### Rješavanje problema sa vezom na serveru

CloseCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, CloseCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa CloseCode karakteristikama.

---

### Obavještenja se ne prikazuju

CloseCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za CloseCode u postavkama vašeg OS-a, i
...
1. Zatvorite CloseCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka CloseCode Desktop aplikacije):
...
Ako imate problema s CloseCode:

1. **Prijavite probleme na GitHub**

   Najbolji način da prijavite greške ili zatražite funkcije je putem našeg GitHub spremišta:

   [**github.com/reubenzhuyt-cloud/CloseCode/issues**](https://github.com/reubenzhuyt-cloud/CloseCode/issues)
...
Da saznate kojim modelima imate pristup, pokrenite `closecode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\closecode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. CloseCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\closecode`

2. Ponovo pokrenite CloseCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti CloseCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
CloseCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
