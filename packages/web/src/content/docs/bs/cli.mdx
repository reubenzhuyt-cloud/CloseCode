---
title: CLI
description: CloseCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CloseCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
closecode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa CloseCode.
...
Pokrenite CloseCode terminalski korisnički interfejs.
...
CloseCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte CloseCode agentima.
...
Priključite terminal na već pokrenut CloseCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim CloseCode backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `closecode`) |
...
CloseCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `closecode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
closecode auth login
```

Kada se CloseCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u CloseCode.
...
Pokrenite CloseCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `closecode serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti CloseCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `closecode`) |
...
Pokrenite CloseCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
closecode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti CloseCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `closecode`).
...
Upravljajte CloseCode sesijama.
...
Prikaži statistiku upotrebe tokena i troškova za vaše CloseCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili CloseCode dijeljenog URL-a.

```bash
closecode import <file>
```

Možete uvesti iz lokalne datoteke ili CloseCode dijeljenog URL-a.
...
Pokrenite CloseCode headless server sa web interfejsom.

```bash
closecode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup CloseCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `closecode`).
...
Deinstalirajte CloseCode i uklonite sve povezane datoteke.
...
Ažurira CloseCode na najnoviju verziju ili određenu verziju.
...
CloseCode CLI prihvata sljedeće globalne zastavice.
...
CloseCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `closecode`) |
