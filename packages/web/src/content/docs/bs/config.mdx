---
title: Konfiguracija
description: Korištenje CloseCode JSON konfiguracije.
---

Možete konfigurirati CloseCode koristeći JSON konfiguracijski fajl.

---

## Format

CloseCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu CloseCode konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se CloseCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Možete konfigurirati postavke servera za naredbe `closecode serve` i `closecode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš CloseCode server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj CloseCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, CloseCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, CloseCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`closecode run`), desktop aplikaciju i GitHub Action.
...
CloseCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, CloseCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju CloseCode sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite CloseCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
