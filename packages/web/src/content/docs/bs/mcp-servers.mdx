...
Možete dodati vanjske alate u CloseCode koristeći _Model Context Protocol_, ili MCP. CloseCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [CloseCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
CloseCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, CloseCode će:
...
Ako server zahtijeva autentifikaciju, CloseCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `closecode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, CloseCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u CloseCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz CloseCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete CloseCode sa vašim Sentry nalogom.
