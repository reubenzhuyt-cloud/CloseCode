---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u CloseCode.
---

import config from "../../../../config.mjs"
export const console = config.console

CloseCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj CloseCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem CloseCode
config.
...
CloseCode Zen je lista modela koje je obezbedio CloseCode tim koji su bili
testirano i potvrđeno da dobro radi sa CloseCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa CloseCode Zen.
:::
...
Radi kao i svaki drugi provajder u CloseCode i potpuno je opcionalan za korištenje.

---

## CloseCode Go

CloseCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža CloseCode tim i koji su testirani i verificirani da dobro rade s CloseCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `CloseCode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s CloseCode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u CloseCode.
:::
...
da pohrani token u closecode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
CloseCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate CloseCode
...
Za korištenje Google Vertex AI s CloseCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki CloseCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki CloseCode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za CloseCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s CloseCode:
...
3. Kopirajte API ključ za korištenje u CloseCode.
...
6. **Važno**: Prije upotrebe modela oblaka u CloseCode, morate lokalno povući informacije o modelu:
...
CloseCode Zen je lista testiranih i verifikovanih modela koju je obezbedio CloseCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>CloseCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **CloseCode Zen**.

```txt
   /connect
```

3. Unesite svoj CloseCode API ključ.
...
Polja `limit` omogućavaju CloseCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `closecode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite CloseCode konfiguraciju i:
