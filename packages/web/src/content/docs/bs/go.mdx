...
CloseCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u CloseCode-u. Pretplatite se na CloseCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili CloseCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa CloseCode-om.
:::
...
CloseCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

CloseCode Go radi kao bilo koji drugi provajder u CloseCode-u.

1. Prijavite se na **<a href={console}>CloseCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `CloseCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na CloseCode Go.
:::
...
CloseCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj CloseCode konfiguraciji
...
Napravili smo CloseCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz CloseCode koristite i bilo kojeg drugog provajdera.
