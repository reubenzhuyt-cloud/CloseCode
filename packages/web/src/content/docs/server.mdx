...
The `closecode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `closecode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `closecode serve` and `closecode web`.
...
When you run `closecode` it starts a TUI and a server. Where the TUI is the
...
You can run `closecode serve` to start a standalone server. If you have the
opencode TUI running, `closecode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the CloseCode [IDE](/docs/ide) plugins.
