...
자세한 디버그 정보를 얻기 위해 `--log-level` 명령줄 옵션을 사용하여 로그 레벨을 설정할 수 있습니다. 예를 들면, `closecode --log-level DEBUG`.
...
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.local\share\closecode`
...
- macOS만: `CloseCode` 메뉴 -> **웹뷰 **(UI가 공백/frozen인 경우).
...
- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.cache\closecode`
...
3. 당신은 `closecode upgrade`를 가진 최신 버전이 있는 것을 지킵니다
...
당신이 액세스 할 수있는 모델을 파악하려면, `closecode models`를 실행
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.local\share\closecode`
...
Windows에서, `WIN+R`를 누르고 삭제하십시오: `%USERPROFILE%\.cache\closecode`

2. 최신 공급자 포장을 재설치하는 Restart closecode
