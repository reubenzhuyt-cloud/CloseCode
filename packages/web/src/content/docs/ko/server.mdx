---
title: 서버
description: HTTP를 통해 CloseCode 서버와 상호 작용합니다.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`closecode serve` 명령은 opencode 클라이언트가 사용할 수 있는 OpenAPI 엔드포인트를 노출하는 headless HTTP 서버를 실행합니다.
...
`OPENCODE_SERVER_PASSWORD`를 설정하여 서버를 HTTP Basic auth로 보호합니다. `closecode` 또는 `OPENCODE_SERVER_USERNAME`를 오버라이드로 설정하는 사용자의 기본값. 이것은 `closecode serve`와 `closecode web` 둘 다에 적용합니다.
...
`closecode`를 실행하면 TUI와 서버를 시작합니다. TUI는 어디에 있습니까?
...
독립 서버를 시작하려면 `closecode serve`를 실행할 수 있습니다. 당신이 있는 경우에
opencode TUI 실행, `closecode serve` 새로운 서버를 시작합니다.
...
[`/tui`](#tui) 엔드포인트는 서버를 통해 TUI를 구동하는 데 사용될 수 있습니다. 예를 들어 미리 작성하거나 프롬프트를 실행할 수 있습니다. 이 설정은 closecode [IDE](/docs/ide) 플러그인에 의해 사용됩니다.
