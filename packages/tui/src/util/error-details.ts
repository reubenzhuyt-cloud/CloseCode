export function errorDetails(input: { title: string; error: string; context?: string; diagnosticRef?: string }) {
  ...
  return {
    text,
    prompt: [
      "Investigate why this CloseCode component failed in the current project.",
      text,
      ...(input.diagnosticRef
        ? [
        ...
        : []),
      "Inspect the relevant project and global CloseCode configuration, startup or loading behavior, required environment variables or credentials, dependencies, and logs. Identify the root cause and recommend a fix. Do not expose credentials.",
    ].join("\n\n"),
  }
}
