      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>CloseCode {app.version}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string, version: string) {
  ...
  const url = new URL("https://github.com/reubenzhuyt-cloud/CloseCode/issues/new?template=bug-report.yml")
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the CloseCode crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n… (truncated)"
  const head = `The CloseCode TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
